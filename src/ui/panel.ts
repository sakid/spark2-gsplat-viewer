...
import { asNumber } from './panel/dom';
import { createLightRow } from './panel/lights';
import { bindRendererSettingsInputs } from './panel/events';
import { orderOutlinerItems } from './panel/outliner';
...
export interface ControlPanel {
  ...
  setVrButton: (button: HTMLElement | null) => void;
}
...
export function createPanel(): ControlPanel {
  ...
  const renderOutlinerList = (): void => {
    ...
    const ordered = orderOutlinerItems(allItems, allItems);
  ...
  bindRendererSettingsInputs(
    rendererSettingsInputs,
    getRendererLightingSettings,
    (settings) => rendererSettingsChangedHandler(settings as RendererLightingSettings)
  );
